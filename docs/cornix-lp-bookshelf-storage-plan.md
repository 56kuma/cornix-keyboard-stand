...
| Bambu Lab でのプリント | 外周と同じ八角形アウトラインの `5.0 mm` 一体ソリッドベースで、格子・溝・ストッパー・支柱が台座へ連続するようにする |
...
- 台座: 外周と同じ八角形アウトラインの `5.0 mm` 一体ソリッドベースにする。支柱まわりの空間、薄い膜だけの部分、孤立して見える四隅ブロックをなくす。
- 左右外周: 連続した壁やレールは作らず、片側 2 本の斜め支柱だけで保持する。
- 中央ディバイダー: 外観はベース面から立ち上がる先端を丸めた 2 枚フィンのまま、低いレールと 2 枚フィンを 1 つの連続スイープ形状として生成する。
- 補強: 大外側の短い補強支柱は削除し、保持は外周の片側 2 本支柱と中央 2 枚フィンへ集約する。

全部を格子にすると、材料は減るがぴったり感や強度が落ちる。今回は「支柱まわりを全部つなげる」「八角形内側に薄い膜だけの面を残さない」という方針に切り替えた。土台は外周と同じ `10.0 mm` 角落としの八角形を、`0.0 mm` から `5.0 mm` まで一体ソリッドで生成する。外周側の保持レールは全長の壁ではなく、左右それぞれ `1/φ²` と `1/φ` の位置に 2 本の独立した斜め支柱だけを置く。各支柱は中央フィンと同じ `4.5 mm` 幅で、ベース面から連続して立ち上げる。中央保持部は同じ黄金比位置に 2 枚フィンを置き、低いレールからフィンへ垂直に切り替わる 1 つの連続メッシュにした。側面はまっすぐ、先端だけ `3.0 mm` の肩落ちで丸めた三角形状にする。
...
Bambu Lab 向けに、底部溝、前後ストッパー、外周支柱、中央フィンはすべて `5.0 mm` 厚の一体八角形ベースへ連続させている。外周は四隅の独立ブロックではなく連続八角形にし、八角形内側もベース厚みまで埋めたため、薄い膜だけの箇所はない。格子の穴は土台からなくし、印刷途中に空中から始まる島や、完成後に独立する小パーツは作らない方針。中央フィンは、以前のような低い補強ブロックとフィンの重なりをやめ、`z=0.0 mm` から立ち上がる低いレールと 2 枚フィンを同一スイープ内でつなげた。赤丸で示された中央根元の空間は、斜めガセットを使わず、同じ連続メッシュ内の垂直切り替えで別シェル化を抑える構造にした。
