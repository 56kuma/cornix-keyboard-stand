...
| 挿し込み口の広げ量 | 底部溝を基準に、中心側へ `4.0 mm`、外側へ `7.0 mm` 広げる |
| スキャン短辺最大 `111.3 mm` | 台座奥行きの安全余裕として残す |
| スキャン厚み最大 `26.4 mm` | 写真実測より大きいため、今回はスロット幅には使わず参考値として残す |
| Cornix Bento Box の `D120 x T55 mm` | 高さは 55 mm に維持。奥行きは張り出し込み最大外形に合わせて 129.3 mm に設定 |
...
| 材料費削減 | 台座、側壁、中央ディバイダーを格子フレーム化 |

## 6. 格子状デザイン

材料費と見た目のため、前版の面で支えるブレードを格子フレームへ変更した。

格子化した箇所:

- 台座: `0.6 mm` の薄い一体底スキンの上に、外周フレーム、短辺方向のクロスバー、斜めクロスバー、溝下土台を載せる。
- 左右側壁: 下側 15 mm はぴったり保持用の連続レール、上側は縦ポストと斜め格子。
- 中央ディバイダー: 下側は連続レール、上側は格子と山型キャップ。
- 補強: 黄金比位置の外側リブを維持。

全部を格子にすると、材料は減るがぴったり感が落ちる。今回は「底面で一体性、下側で精度、上側で軽量化と見た目」という分担にした。底面スキンは薄く抑え、Apple 製品のような一体感のある影を作りつつ、上面は格子で軽く見せる。
...
台座は `129.3 / 74.2 = 1.74` で、厳密な黄金比より少し奥行きを優先している。これはスキャン短辺最大 `111.3 mm` と前後ストッパーを安全に収めるため。全体高さは `144.0 / φ² = 55.0 mm` に近く、縦置きした長辺矩形部に対して黄金比の下側支持高さになる。実際に挟むスロット長は、写真実測の短辺矩形部 `88.0 mm` を基準に `88.8 mm` とした。厚み方向は実測 `10.0 mm` を基準に、底部だけ `10.8 mm` 幅・`1.0 mm` 高さのはめ込み溝で位置決めする。挿し込み口は中心側へ `4.0 mm`、外側へ `7.0 mm` 広げ、全体の導入口幅を `21.8 mm` にした。

Bambu Lab 向けに、底部溝の真下と前後ストッパーの真下には台座厚みまで続く土台を入れている。格子の穴は残すが、印刷途中に空中から始まる島や、完成後に独立する小パーツは作らない方針。
...
格子状なので、見た目重視ならマット系フィラメントが合う。材料費をさらに抑えるなら、壁数 3、インフィル 15% から試す。ただし前後ストッパー、底部溝、下側レールの強度を落としすぎないこと。
...
- 挿し込み口内寸 `88.8 x 21.8 mm` で差し込みやすいか。
- USB-C ポートや電源スイッチが格子壁に当たらないか。
...
- 格子をもっと軽くしたい: `LATTICE_BAR` を少し細くする。
- 格子をもっと強くしたい: `LATTICE_BAR` を太くするか、`LATTICE_VERTICALS` を増やす。
