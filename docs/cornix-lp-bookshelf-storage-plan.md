...
| フック状ズレ止め | 前後ストッパーに 1.0 mm の低いフック形状を入れる |
...
- 台座: 全面プレートではなく、外周フレーム、短辺方向のクロスバー、斜めクロスバーで構成。
- 左右側壁: 下側 15 mm はぴったり保持用の連続レール、上側は縦ポストと斜め格子。
- 中央ディバイダー: 下側は連続レール、上側は格子と山型キャップ。
- 補強: 黄金比位置の外側リブを維持。

全部を格子にすると、材料は減るがぴったり感が落ちる。今回は「下側で精度、上側で軽量化と見た目」という分担にした。
...
台座は `129.3 / 74.2 = 1.74` で、厳密な黄金比より少し奥行きを優先している。これはスキャン短辺最大 `111.3 mm` と前後ストッパーを安全に収めるため。全体高さは `144.0 / φ² = 55.0 mm` に近く、縦置きした長辺矩形部に対して黄金比の下側支持高さになる。実際に挟むスロット長は、写真実測の短辺矩形部 `88.0 mm` を基準に `88.8 mm` とした。厚み方向は実測 `10.0 mm` を基準に、底部だけ `10.8 mm` 幅・`1.0 mm` 高さのはめ込み溝で位置決めし、上部は `12.8 mm` 幅にして差し込みやすくした。
...
格子状なので、見た目重視ならマット系フィラメントが合う。材料費をさらに抑えるなら、壁数 3、インフィル 15% から試す。ただし前後フックと下側レールの強度を落としすぎないこと。
...
- スロット上部内寸 `88.8 x 12.8 mm` で差し込みやすいか。
